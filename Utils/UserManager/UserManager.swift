...
@objcMembers class UserManager: NSObject {
    ...
    /// 默认车辆
    @objc var defaultCar: Car? {
        get {
            guard let tmpCars = _cars , !tmpCars.isEmpty else  {
                return nil
            }
            for (car) in tmpCars {
                if car.isOnLine {
    ...
    ///
    @objc dynamic var time: NSString?
    
    ///
    @objc dynamic var speed: NSNumber?
    
    
    // MARK: 当前车辆其他状况
    ///续航电量
    @objc dynamic var lifeScale: NSNumber?
    
    ///今日骑行时间
    @objc dynamic var todayRidingTime: NSNumber?
    
    ///今日里程
    @objc dynamic var todayMileage: NSNumber?
    
    
    ///小计里程
    @objc dynamic var subMileage: NSNumber?
    
    ///续航里程
    @objc dynamic var lifeMileage: NSNumber?
    
    
    ///是否有新报警
    @objc dynamic var hasNewAlert: NSNumber {
        get {
            let userDefault = UserDefaults.standard
            let hasNewAlert = userDefault.bool(forKey: kHasNewAlert)
            if hasNewAlert {
                return 1
            } else {
                return 0
        ...
        set {
            let userDefault = UserDefaults.standard
            let valueOne = newValue.boolValue
            userDefault.set(valueOne, forKey: kHasNewAlert)
            userDefault.synchronize()
        }
    }
    
    ///是否有新故障
    @objc dynamic var hasNewFailure: NSNumber {
        get {
            let userDefault = UserDefaults.standard
            let hasNewAlert = userDefault.bool(forKey: kHasNewFailure)
            if hasNewAlert {
                return 1
            } else {
                return 0
        ...
        set {
            let userDefault = UserDefaults.standard
            let valueOne = newValue.boolValue
            userDefault.set(valueOne, forKey: kHasNewFailure)
            userDefault.synchronize()
        }
    }
    
    ///是否有新消息
    @objc dynamic var hasNewMessage: NSNumber {
        get {
            let userDefault = UserDefaults.standard
            let hasNewAlert = userDefault.bool(forKey: kHasNewMessage)
            if hasNewAlert {
                return 1
            } else {
                return 0
        ...
        set {
            let userDefault = UserDefaults.standard
            let valueOne = newValue.boolValue
            userDefault.set(valueOne, forKey: kHasNewMessage)
            userDefault.synchronize()
        }
    }
        ...
        ///用户所有车辆
    fileprivate var _cars: [Car]?
    @objc dynamic var cars: [Car]? {
        get {
            return _cars
        }
        set {
...
//    dynamic var telephone:String?
    ...
    func setup(){
//        telephone = YILSettings.shared[kCurrentUser]
    }
