...
class YILUsersKeychain {
    static let sharedInstance = YILUsersKeychain()
...
//        keychain.set(data.base64EncodedData(options: .endLineWithCarriageReturn), forKey: YILUsersKeychain.usersKey)
        
    }
    
    func getUsersDict() -> [String:LocalSecurityAccountModel]{
        if users.count <= 0 {
//            let data = keychain.getData(YILUsersKeychain.usersKey)
//            if let data = data{
